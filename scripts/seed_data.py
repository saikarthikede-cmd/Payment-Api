...
def signin(email: str, password: str = DEFAULT_PASSWORD):
...
def seed_wallet(customer_id: str, token: str, initial_balance: float = 1000.0):
...
def seed_orders(customer_id: str, token: str, count: int = 3):
...
def seed_multiple_users():
    ...
    for user_id, email, full_name, phone in users:
        ...
        if seed_user(user_id, email, full_name, phone):
            token = signin(email)
            if token:
                seed_wallet(user_id, token, 1000.0 + (int(user_id.split('-')[1]) * 500))
                seed_orders(user_id, token, 2)
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--all":
        ...
        if seed_user(customer_id, email, full_name, "+91-9876543210"):
            token = signin(email)
            if token:
                seed_wallet(customer_id, token, 1000.0)
                seed_orders(customer_id, token, 3)
